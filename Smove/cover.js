...
var drawFrontCover = function()
{
    ...
    ctx.font = 'bold 60px Moonlight';
    var title = "SMOVE";

    //Smove字样外面的圆角矩形
    ctx.strokeStyle = "#F0FFFF";
    ctx.lineWidth = 4;
    frontCoverRectX = (gameWidth - ctx.measureText(title).width - 80) / 2;
    frontCoverRectY = (gameHeight + 60 - 40 - 60 - 80) / 2;
    frontCoverRectWidth =  ctx.measureText(title).width + 80;
    frontCoverRectHeight =  60 + 80/2;
    ctx.roundRect(frontCoverRectX, frontCoverRectY, frontCoverRectWidth, frontCoverRectHeight, 50).stroke();
    ...
    ctx.fillText(title,(gameWidth - ctx.measureText(title).width)/2,(gameHeight + 60 - 40)/2);
...
var drawBackCover = function()
{
    ...
    //绘制得分
    var title = score+"";
    ctx.fillText(title,(gameWidth - ctx.measureText(title).width)/2,(gameHeight + 60 - 40)/2 - 100);

    title = "GAME OVER";
    //绘制GAME OVER
    ctx.fillText(title,(gameWidth - ctx.measureText(title).width)/2,(gameHeight + 60 - 40)/2 - 40);

    //绘制Restart字样外面的圆角矩形
    title = "Restart"
    ctx.font = 'bold 40px Moonlight';

    ctx.strokeStyle = "#F0FFFF";
    ctx.lineWidth = 4;
    backCoverRectX = (gameWidth - ctx.measureText(title).width - 80) / 2;
    backCoverRectY = (gameHeight + 40 - 40 - 40 - 80) / 2 + 45;
    backCoverRectWidth =  ctx.measureText(title).width + 80;
    backCoverRectHeight =  40 + 80/2;
    ctx.roundRect(backCoverRectX, backCoverRectY, backCoverRectWidth, backCoverRectHeight, 40).stroke();

    //绘制Restart字样
    ctx.fillText(title,(gameWidth - ctx.measureText(title).width)/2,(gameHeight + 40 - 40)/2 + 40);
}
